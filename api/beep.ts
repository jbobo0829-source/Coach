// 440 Hz beep; duration via ?ms=... (50–2000). Runs on Vercel Edge.
...
function renderToneWav(hz: number, ms: number, volume = 0.9, sampleRate = 44100): Uint8Array {
  ...
  // Generate sine wave with short fade in/out to avoid clicks
  for (let i = 0; i < samples; i++) {
    const t = i / sampleRate;
    const attack = sampleRate * 0.01; // 10ms
    const release = sampleRate * 0.01; // 10ms
    const env =
      Math.min(1, i / attack) *
      Math.min(1, (samples - 1 - i) / release);
    const s = Math.sin(twoPiF * t) * volume * env;
    data[i] = Math.max(-32768, Math.min(32767, Math.floor(s * 32767)));
  }

  // Build a minimal PCM 16-bit mono WAV header
...
export default async function handler(req: Request) {
  try {
    ...
    const ms = clamp(parseInt(url.searchParams.get('ms') || '300', 10), 50, 2000);
    const wav = renderToneWav(440, ms);
    ...
    return new Response('Error generating beep', { status: 500 });
  }
}
